  ...
  describe "self.get method" do
    ...
    it "raises SlackAPIError for bad API call" do
      VCR.use_cassette("self.get API error") do
        expect{SlackCLI::Recipient.get("users.list", {token: "sggreg"})}.must_raise SlackCLI::SlackAPIError
      end
    end

    it "returns HTTParty::Response for succesful calls" do
      VCR.use_cassette("self.get nominal") do
        expect(SlackCLI::Recipient.get("users.list", {token: ENV["SLACK_API_TOKEN"]})).must_be_instance_of HTTParty::Response
      end
    end
  end
